...
export function Provider(props: ProvidersProps) {
  ...
  function handleUpdate() {
    show()
    client
      .updateRuleProvider(provider.name)
      .then(async () => await update())
      .finally(() => hide())
  }

  const updateClassnames = classnames('rule-provider-icon', {
    'rule-provider-loading': visible,
  })
    ...
    <div className="rule-provider">
      <div className="rule-provider-header">
        <div className="rule-provider-header-part">
          ...
          <span className="rule-provider-update">{`${t('ruleCount')}: ${
            provider.ruleCount
          }`}</span>
        </div>
        <div className="rule-provider-header-part">
          {provider.updatedAt && (
            <span className="rule-provider-update">{`${t(
              'providerUpdateTime'
            )}: ${fromNow(new Date(provider.updatedAt), lang)}`}</span>
          )}
          <Icon
