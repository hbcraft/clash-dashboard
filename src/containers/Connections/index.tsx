...
const shouldCenter = new Set<string>([
  Columns.Network,
  Columns.Type,
  Columns.Speed,
  Columns.Upload,
...
export default function Connections() {
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor(Columns.Host, {
        minSize: 260,
        size: 260,
        header: t(`columns.${Columns.Host}`),
      }),
      columnHelper.accessor(Columns.Network, {
        minSize: 80,
        size: 80,
        header: t(`columns.${Columns.Network}`),
      }),
      columnHelper.accessor(Columns.Type, {
        minSize: 100,
        size: 100,
      ...
      columnHelper.accessor(Columns.Rule, {
        minSize: 140,
        size: 140,
        header: t(`columns.${Columns.Rule}`),
      }),
      columnHelper.accessor(Columns.Process, {
        minSize: 100,
        size: 100,
        header: t(`columns.${Columns.Process}`),
        cell: (cell) => (cell.getValue() ? basePath(cell.getValue()!) : '-'),
      }),
      columnHelper.accessor((row) => [row.speed.upload, row.speed.download], {
        id: Columns.Speed,
      ...
      columnHelper.accessor(Columns.Upload, {
        minSize: 100,
        size: 100,
        header: t(`columns.${Columns.Upload}`),
        cell: (cell) => formatTraffic(cell.getValue()),
      }),
      columnHelper.accessor(Columns.Download, {
        minSize: 100,
        size: 100,
        header: t(`columns.${Columns.Download}`),
        cell: (cell) => formatTraffic(cell.getValue()),
      }),
      columnHelper.accessor(Columns.SourceIP, {
        minSize: 140,
        size: 140,
        header: t(`columns.${Columns.SourceIP}`),
        filterFn: 'equals',
      }),
      columnHelper.accessor(Columns.Time, {
        minSize: 120,
        size: 120,
        header: t(`columns.${Columns.Time}`),
        cell: (cell) => fromNow(new Date(cell.getValue()), lang),
        sortingFn: (rowA, rowB) =>
          (rowB.original?.time ?? 0) - (rowA.original?.time ?? 0),
      }),
    ],
    [lang, t]
  )
  ...
  useEffect(() => {
    ...
    if (conn) {
      ...
      Object.keys(latestConntion.current).length !== 0 &&
      !latestConntion.current.completed
    ) {
      setDrawerState((d) => {
        d.connection.completed = true
      })
  ...
  const content = instance.getRowModel().rows.map((row) => {
    return (
      <tr
        ...
        key={row.original?.id}
        onClick={() =>
          setDrawerState({ visible: true, selectedID: row.original?.id })
        }
      >
        {row.getAllCells().map((cell) => {
          const classname = classnames(
            'connections-block',
            {
              'text-center': shouldCenter.has(cell.column.id),
    ...
    <div className="page !h-100vh">
      <Header title={t('title')}>
        <span className="connections-filter flex-1 cursor-default">
          {`(${t('total.text')}: ${t('total.upload')} ${formatTraffic(
            traffic.uploadTotal
          )} ${t('total.download')} ${formatTraffic(traffic.downloadTotal)})`}
        </span>
        <Checkbox
      ...
      <Card ref={cardRef} className="connections-card relative">
        <div className="min-h-full min-w-full overflow-auto">
          <table>
            <thead>
              <tr className="connections-header">{headers}</tr>
            </thead>
            <tbody>{content}</tbody>
          </table>
        </div>
      ...
      >
        ...
        <div className="mt-3 flex justify-end pr-3">
          <Button
            type="danger"
            disiabled={drawerState.connection.completed}
            onClick={() => handleConnectionClosed()}
          >
            {t('info.closeConnection')}
          </Button>
        </div>
      </Drawer>
    </div>
