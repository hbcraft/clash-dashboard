...
export type Connection = API.Connections & {
  completed?: boolean
  uploadSpeed: number
...
class Store {
  ...
  appendToSet(connections: API.Connections[]) {
    const mapping = connections.reduce(
      ...
      new Map<string, API.Connections>()
    )
    ...
    for (const id of mapping.keys()) {
      ...
      this.connections?.set(id, {
        ...n,
        uploadSpeed: n.upload - c.upload,
        downloadSpeed: n.download - c.download,
      })
    }
  }
...
export function useConnections() {
  ...
  const feed = useCallback(
    function (connections: API.Connections[]) {
