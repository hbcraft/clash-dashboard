...
export default function Logs() {
  ...
  useLayoutEffect(() => {
    ...
    if (
      ul != null &&
      scrollHeightRef.current === ul.scrollTop + ul.clientHeight
    ) {
  ...
  useEffect(() => {
    function handleLog(newLogs: Log[]) {
      logsRef.current = logsRef.current
        .slice()
        .concat(newLogs.map((d) => ({ ...d, time: new Date() })))
  ...
  return (
    <div className="page">
      <Header title={t('title')}>
        <span className="text-primary-darken mr-2 text-sm">
          {t('levelLabel')}:
        </span>
        <Select
        ...
        <ul className="logs-panel" ref={listRef}>
          {logs.map((log, index) => (
            <li className="inline-block text-[11px] leading-5" key={index}>
              ...
              <span className={logMap.get(log.type)}>
