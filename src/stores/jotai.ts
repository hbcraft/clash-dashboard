...
export function useRuleProviders() {
  ...
  const { data, mutate } = useSWR(
    ...
    async () => {
      ...
      return Object.keys(ruleProviders.data.providers).map<API.RuleProvider>(
...
export function useLogsStreamReader() {
  ...
  const previousKey = usePreviousDistinct(
    `${apiInfo.protocol}//${apiInfo.hostname}:${
      apiInfo.port
    }/logs?level=${level}&secret=${encodeURIComponent(apiInfo.secret)}`
  )
  ...
  useEffect(() => {
    if (level) {
      ...
      const logUrl = `${protocol}//${apiInfo.hostname}:${
        apiInfo.port
...
export function useConnectionStreamReader() {
  ...
  const protocol = apiInfo.protocol === 'http:' ? 'ws:' : 'wss:'
  const url = `${protocol}//${apiInfo.hostname}:${
    apiInfo.port
